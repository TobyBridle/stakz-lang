    ...
    if(parser->token->stack_type == TOKEN_OPERATOR && parser->stacks->op != NULL && parser->token->type != TOKEN_EOF)
    {
            ...
            switch(keyword)
            {
                case RETURN:
                    ...
                    if(parser->stacks->output->value == NULL)
                    {
                    ...
                    */
                    ...
                    // 1. Pop return code from Stack
                    void* return_code = parser->stacks->output->value;
                
                    // 2. Check return code is Valid
                    if(parser->stacks->output_token_type->value == TOKEN_STRING)
                    {
